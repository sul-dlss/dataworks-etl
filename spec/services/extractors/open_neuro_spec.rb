...
RSpec.describe Extractors::OpenNeuro do
  context 'when successful' do
    ...
    let!(:existing_dataset_record) do
      create(:dataset_record, dataset_id: '10.17632/9pxgdp7cn9', provider: 'open_neuro')
    end
    ...
    it 'creates a dataset record set' do
      ...
      expect(client).to have_received(:list).with(affiliation: 'Stanford University', client_id: 'sul.openneuro')
      ...
      expect(new_dataset_record.provider).to eq('open_neuro')
      ...
      expect(dataset_record_set.provider).to eq('open_neuro')
